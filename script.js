...
document.querySelectorAll('.add-btn').forEach(button => {
    button.addEventListener('click', (e) => {
        ...
        Swal.fire({
    title: 'Added to Cart!',
    text:item.name+ 'is ready for checkout.',
    icon: 'success',
    timer: 1400, // Closes after 2 seconds
...
function showReviewPopup() {
  ...
  Swal.fire({
    title: 'NOPE..',
    text: 'seems like your cart is lonely',
    icon: 'error',
...
// Run this when the home page opens
window.onload = function() {
    const cartContainer = document.getElementById('homeCartDisplay');
    const savedCart = JSON.parse(localStorage.getItem('myCart')) || [];

    if (savedCart.length > 0) {
        // Clear a placeholder if it exists
        cartContainer.innerHTML = ''; 

        savedCart.forEach(item => {
            const itemHTML = `
                <div class="cart-item-home">
